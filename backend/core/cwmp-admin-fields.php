...
if(esc_html(get_option('cwmp_activate_checkout'))=="S"){
	...
	function cwmp_add_custom_fields($fields) {
		...
		if ($results) {
			foreach ($results as $row) {
				...
				$fields['billing']['billing_'.esc_html($row->name)] = array(
					'label'       => esc_html($row->label),
					'placeholder' => esc_html($row->placeholder),
					'required'    => $required,
					'clear'       => true,
				);
			}
	...
	function cwmp_add_field_name(){
		...
		if ($results) {
			foreach ($results as $row) {
				if($row->type=="text"){
				?>
				<p class="cwmp-form-row validate-required" id="billing_<?php echo esc_html($row->name); ?>_field">
					<label for="billing_<?php echo esc_html($row->name); ?>" class="screen-reader-text"><abbr class="required" title="obrigatório">*</abbr></label>
					<span class="woocommerce-input-wrapper">
						<input type="text" class="input-text" name="billing_<?php echo esc_html($row->name); ?>" id="billing_<?php echo esc_html($row->name); ?>" placeholder="<?php echo esc_html($row->placeholder); ?>" value="<?php echo esc_html($row->default_value); ?>" />
					</span>
				...
				?>
				<p class="cwmp-form-row is-active" id="billing_<?php echo esc_html($row->name); ?>_field">
					<label for="billing_<?php echo esc_html($row->name); ?>" class="screen-reader-text"><?php echo esc_html($row->label); ?>&nbsp;<abbr class="required" title="obrigatório">*</abbr></label>
					<span class="woocommerce-input-wrapper">
						<select name="billing_<?php echo esc_html($row->name); ?>" id="billing_<?php echo esc_html($row->name); ?>" class="select" data-placeholder="<?php echo esc_html($row->placeholder); ?>" autocomplete="off">
							<?php
	...
	function cwmp_add_field_update($order_id) {
		...
		if ($results) {
			foreach ($results as $row) {
				if (isset($_POST['billing_'.$row->name])) {
					$value = sanitize_text_field(wp_unslash($_POST['billing_'.$row->name]));
				} else {
					$value = '';
				}
	...
	function cwmp_custom_checkout_field_display_admin_order_meta($order) {
		...
		echo '<h4>' . esc_html__('Campos Adicionais', 'woocommerce') . '</h4>';
		global $wpdb;
		$results = $wpdb->get_results("SELECT * FROM {$wpdb->prefix}cwmp_fields");
		if ($results) {
			foreach ($results as $row) {
				echo '<p><strong>' . esc_html($row->label) . ':</strong> ' . esc_html(get_post_meta($order->get_id(), $row->name, true)) . '</p>';
			}
		}
		echo '</div>';
	}
