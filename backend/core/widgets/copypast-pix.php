...
class Copy_Past_Pix extends \Elementor\Widget_Base {
	...
	protected function render() {
		...
		echo "
		<textarea class='copypast' rows='".esc_html($settings['cwmp_qrcode_pix_size'])."'>" . esc_html($code_pix) . "</textarea>
		<button class='buttoncopypast' style='border:0px;'>".esc_html($settings['cwmp_qrcode_pix_button_text'])."</button>
		...
		";
			...
			echo "
			<textarea class='copypast' rows='".esc_html($settings['cwmp_qrcode_pix_size'])."'>00020126350014br.gov.bcb.pix0113j@rjsc.com.br5204000053039865406116.995802BR5924JARDELJUNIORJARDELJUNIOR6006Niteri62240520mpqrinter530739545526304C1FF</textarea>
			<button class='buttoncopypast'>".esc_html($settings['cwmp_qrcode_pix_button_text'])."</button>
			<p style='display:none' class='return_copy'>Código Copiado</p>
			<script type='text/javascript'>
			jQuery(document).ready(function($) {
			$('.buttoncopypast').click(function(){
