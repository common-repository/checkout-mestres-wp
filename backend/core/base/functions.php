...
function cwmpListBuyProduct($product_id, $data_especifica, $status) {
    $data_especifica = gmdate('Y-m-d', strtotime($data_especifica));
...
function cwmpCountSendRecoveryCart($id){
    global $wpdb;
	global $table_prefix;
	$get_verify_cart = $wpdb->get_results("SELECT * FROM {$wpdb->prefix}cwmp_cart_abandoned_relation WHERE cart='".$id."'");
	return count($get_verify_cart);
}
function cwmp_check_order_by_recovery_cart($recovery_cart_value) {
    ...
    $args = array(
        ...
        'meta_query'  => array(
            array(
                'key'     => 'cwmpRecoveryCart', 
                'value'   => $recovery_cart_value, 
                'compare' => '='
            ),
        ),
        'numberposts' => 1,
