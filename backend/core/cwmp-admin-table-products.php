...
add_filter( 'woocommerce_shop_order_list_table_columns', function ( $columns ) {
	...
	foreach ( $columns as $key => $value ) {
		if ( $key === 'order_total' ) {
			$new_columns['cwmp-security'] = 'Segurança';
			$new_columns['cwmp-whatsapp'] = 'WhatsApp';
			$new_columns['cwmp-view-rastreio'] = 'Rastreio';
		}
...
add_action( 'woocommerce_shop_order_list_table_custom_column', function ( $column, $order ) {
	$cwmp_order_info = wc_get_order($order->get_id());
	if ( $column == "cwmp-security") {
		$pontos = 0;
	...
	if ( $column == "cwmp-whatsapp") {
		echo '<a href="https://api.whatsapp.com/send?phone=55'.esc_html(preg_replace('/[^0-9]/', '', $cwmp_order_info->get_billing_phone())).'" target="blank"><svg width="30" height="31" viewBox="0 0 256 259" fill="none" xmlns="http://www.w3.org/2000/svg">
	...
	if ( $column == "cwmp-view-rastreio") {
		...
		if(!empty($transportadora_id)){
			...
			if(get_post_meta($cwmp_order_info->get_id(), '_cwmp_codigo_rastreio_slug', true)){
				echo "<a href='".esc_html(str_replace("{{track}}", get_post_meta($cwmp_order_info->get_id(), '_cwmp_codigo_rastreio_slug', true),$get_campanha[0]->estrutura))."' target='blank'>".esc_html(str_replace("{track}", get_post_meta($cwmp_order_info->get_id(), '_cwmp_codigo_rastreio_slug', true),$get_campanha[0]->estrutura))."</a>";
			}
		}
	}
...
//not hpos
add_filter( 'manage_edit-shop_order_columns', 'cwmp_custom_shop_order_column', 20 );
function cwmp_custom_shop_order_column( $columns ) {
...
add_action( 'manage_shop_order_posts_custom_column' , 'cwmp_custom_orders_list_column_content', 20, 2 );
function cwmp_custom_orders_list_column_content( $column, $post_id ){
